...
static usb_status_t USB_DeviceHidMouseAction(void)
{
	...
	enum
	{
		...
		QUINCE, DSEIS, DSIETE, DOCHO, DNUEVE, VEINTE,
		VUNO, VDOS, VTRES, VCUATRO, VCINCO, VSEIS, VSIETE,
		VOCHO, VNUEVE, TREINTA, TUNO, TDOS, TTRES
	};
	static uint8_t dir = FIRST_K;

	g_UsbDeviceHidMouse.buffer_key[1] = 0X00U;
	g_UsbDeviceHidMouse.buffer_key[3] = 0x00U;
	g_UsbDeviceHidMouse.buffer_key[4] = 0x00U;
	g_UsbDeviceHidMouse.buffer_key[5] = 0x00U;
	g_UsbDeviceHidMouse.buffer_key[6] = 0x00U;
	g_UsbDeviceHidMouse.buffer_key[7] = 0x00U;
	switch (dir)
	{
	case FIRST_K:
		z++;
		if (z > 100U)
		{
			...
			g_UsbDeviceHidMouse.buffer_key[1] = MODIFERKEYS_LEFT_CTRL;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_ESCAPE;
		}
		break;
	case SECOND_K:
		z--;
	...
	case THIRD_K:
		...
		if (z > 100U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_A;
		}
		break;
	case FOURTH_K:
		z--;
		if (z < 1U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_I;
		}
		break;
	case FIFTH_K:
		z++;
		if (z > 100U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_N;
		}
		break;
	case SIXTH_K:
		z--;
		if (z < 1U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_T;
		}
		break;
	case SEVENTH_K:
		z++;
		if (z > 100U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_ENTER;
		}
		break;
	case DELAY:
		delay++;
	...
	case FIRST:
		...
		g_UsbDeviceHidMouse.buffer[1] = 1U;
	...
	case DOS:
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_B;
		}
		break;
	case TRES:
		z--;
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_L;
		}
		break;
	case CUATRO:
		z++;
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_O;
		}
		break;
	case CINCO:
		z--;
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_C;
		}
		break;
	case SEIS:
		z++;
	...
	case SIETE:
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[1] = MODIFERKEYS_LEFT_GUI;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_LEFTARROW;
		}
		break;
	case OCHO:
		z++;
	...
	case CATORCE:
		...
		if (z > 100U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[1] = MODIFERKEYS_LEFT_GUI;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_RIGHTARROW;
		}
		break;
	case QUINCE:
		delay--;
	...
	case DSEIS:
		...
		g_UsbDeviceHidMouse.buffer[0] = 1U;
		g_UsbDeviceHidMouse.buffer[1] = 0U;
		g_UsbDeviceHidMouse.buffer[2] = (uint8_t)(-4);
		g_UsbDeviceHidMouse.buffer[3] = 0U;
		x++;
		if (x > 200U)
		{
	...
	case DNUEVE:
		...
		if (z < 1U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 0x02U;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_H;
			g_UsbDeviceHidMouse.buffer_key[4] = KEY_O;
			g_UsbDeviceHidMouse.buffer_key[5] = KEY_L;
			g_UsbDeviceHidMouse.buffer_key[6] = KEY_A;
		}
		break;
	case VEINTE:
		z++;
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_SPACEBAR;
		}
		break;
	case VUNO:
		z--;
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_M;
			g_UsbDeviceHidMouse.buffer_key[4] = KEY_U;
			g_UsbDeviceHidMouse.buffer_key[5] = KEY_N;
			g_UsbDeviceHidMouse.buffer_key[6] = KEY_D;
			g_UsbDeviceHidMouse.buffer_key[7] = KEY_O;
		}
		break;
	case VDOS:
		z++;
		if (z > 100U)
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[1] = MODIFERKEYS_LEFT_CTRL;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_E;
		}
		break;
	case VTRES:
		z--;
	...
	case VCUATRO:
		...
		g_UsbDeviceHidMouse.buffer[0] = 1U;
		g_UsbDeviceHidMouse.buffer[1] = 0U;
		g_UsbDeviceHidMouse.buffer[2] = 8U;
		g_UsbDeviceHidMouse.buffer[3] = 0U;
		x--;
		if (x < 1U)
		{
	...
	case VSIETE:
		...
		{
			dir++;
			g_UsbDeviceHidMouse.buffer_key[0] = 2U;
			g_UsbDeviceHidMouse.buffer_key[1] = MODIFERKEYS_LEFT_CTRL;
			g_UsbDeviceHidMouse.buffer_key[3] = KEY_V;
		}
		break;
	default:
		break;
	}
	/* Send mouse or keyboard report to the host */
	if(g_UsbDeviceHidMouse.buffer_key[0] == 2U)
